...
BEGIN_MESSAGE_MAP(CStudentManagementSystemDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &CStudentManagementSystemDlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CStudentManagementSystemDlg::OnBnClickedButton2)
END_MESSAGE_MAP()
...
BOOL CStudentManagementSystemDlg::OnInitDialog()
{
	...
	//MYSQL mysql; //数据库连接句柄
	//mysql_init(&mysql);
	//if (!mysql_real_connect(&mysql, "localhost", "root", "2005", "bjpowernode", 3306, NULL, 0))
	//{        //bjpowernode为你所创建的数据库，3306为端口号，root是用户名,2005是密码
	//	MessageBox(TEXT("数据库连接失败"));
	//	return FALSE;
	//}
	MysqlManagement mysql_opera;
	mysql_opera.CreateDatabase();
...
void CStudentManagementSystemDlg::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CStudentManagementSystemDlg::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
